...
static void funcargs (LexState *ls, expdesc *f) {
  ...
  switch (ls->t.token) {
    ...
    case TK_NUMBER: { /* funcargs -> NUMBER, an addition from LuaPLUS */
      init_exp(&args, VK, luaK_numberK(ls->fs, ls->t.seminfo.r)); // Create a number constant from semantic info, it can't be used in math just like string cant be concatanated.
      luaX_next(ls);
      break;
    }
    default: {
...
static void primaryexp (LexState *ls, expdesc *v) {
  ...
  for (;;) {
    switch (ls->t.token) {
      ...
      case '(': case TK_STRING: case TK_NUMBER: case '{': {  /* funcargs */
...
// Enterance to the lua5.1 parser, after lexing.
static void chunk (LexState *ls) {
  /* chunk -> { stat [`;'] } */
  int islast = 0;
  enterlevel(ls);
